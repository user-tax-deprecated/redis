...
hexist(&self, key:Bin, field:Bin) -> bool {
...
zscore(&self, key:Bin, member:Bin) -> Option<i64> {
  self.0.zscore::<Option<i64>,_,_>(key, member).await?
}

zincrby(&self, key:Bin, member:Bin, increment:f64) -> f64 {
  self.0.zincrby::<f64,_,_>(key, increment,  member).await?
}

zincr(&self, key:Bin, member:Bin) -> f64 {
  self.0.zincrby::<f64,_,_>(key, 1.0, member).await?
}
...
fstr(&self, name:Bin, keys:Option<Vec<Bin>>, vals:Option<Vec<Bin>>) -> Option<String> {
  fcall::<Option<String>>(self,name,keys,vals).await?
}
...
fbool_r(&self, name:Bin, keys:Option<Vec<Bin>>, vals:Option<Vec<Bin>>) -> bool {
  fcall_ro::<bool>(self, name, keys, vals).await?
}

fstr_r(&self, name:Bin, keys:Option<Vec<Bin>>, vals:Option<Vec<Bin>>) -> Option<String> {
  fcall_ro::<Option<String>>(self,name,keys,vals).await?
}
